...
# Function to check for SGID binaries
check_sgid_binaries() {
    echo -e "\e[33m[*] SGID Binaries:\e[0m"
    while IFS= read -r line; do
        permissions=$(echo "$line" | awk '{print $1}')
        file=$(echo "$line" | awk '{print $NF}')
...
check_network() {
    echo -e "\e[33m[*] Checking What's on Local Network:\e[0m"
    netstat -antup
}
...
check_pspy() {
    ...
    echo -e "\e[35m[!] Don't Forget to Run:\e[0m \e[31m Pspy \e[0m"
